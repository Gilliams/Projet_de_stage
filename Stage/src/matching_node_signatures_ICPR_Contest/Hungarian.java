...
public class Hungarian {
	...
	public static float findLargest		//Finds the largest element in a positive array.
	(float[][] array)
	...
	public static int[][] hgAlgorithm (float[][] array, String sumType)
	{
		float[][] cost = copyOf(array);	//Create the cost matrix.
		...
		{
			float maxWeight = findLargest(cost);
		...
		float maxCost = findLargest(cost);		//Find largest cost matrix element (needed for step 6).
...
//		print2DArrayInt(mask, "%d\t", "Mask matrix after convertPath()", 1);
	...
	public static int hg_step6(int step, float[][] cost, int[] rowCover, int[] colCover, float maxCost)
	{
		//What STEP 6 does:
...
//		print2DArray(cost, "%.1f\t", "\nNow the cost matrix becomes:", 0);	
	...
	(float[][] cost, int[] rowCover, int[] colCover, float maxCost)
	{
		float minval = maxCost;	//There cannot be a larger cost than this.
...
//		float totalTime = getTime(startTime, endTime);
